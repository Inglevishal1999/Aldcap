...
// Render backend URL — used only for uploaded images
const API_ORIGIN = "https://elaap-backend-live.onrender.com";

// Images come back from the API as relative paths like:
// "/uploads/gallery/xyz.jpg"
// Convert them to full Render URLs.
...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    async function fetchItems() {
      ...
      try {
        // Uses:
        // https://elaap-backend-live.onrender.com/api/gallery
  ...
  return (
    <div className="bg-[#EEF1F6]">
      ...
      <section className="relative overflow-hidden bg-[#0F2A52] px-6 pb-0 pt-24 text-white">
        ...
        <div className="relative mx-auto max-w-6xl">
          ...
          <h1 className="mt-5 flex items-start gap-2 text-[13vw] font-extrabold uppercase leading-[0.88] tracking-tight sm:text-6xl md:text-7xl">
            ...
            <span>
              Powering the <span className="text-[#F2A93B]">Future</span>
              ,
      ...
      <section className="mx-auto max-w-6xl px-6 py-16">
        ...
        {loading && (
          <p className="py-16 text-center text-[#0F2A52]/50">
            Loading gallery…
          </p>
        )}
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
              <button
                ...
                className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-medium transition-colors ${
                  n === page
                    ? "bg-[#F2A93B] text-[#0F2A52]"
                    : "text-[#0F2A52] hover:bg-[#0F2A52]/5"
                }`}
              >
                {n}
              </button>
        ...
        <div className="mt-16 flex flex-col items-start justify-between gap-6 rounded-2xl bg-[#0F2A52] p-8 text-white sm:flex-row sm:items-center">
          <div className="flex items-center gap-4">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/20 bg-white/5 text-[#F2A93B]">
              ...
              >
                <path
                  d="M3 8a2 2 0 0 1 2-2h2l1.5-2h7L17 6h2a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2Z"
                  strokeLinejoin="round"
                />
            ...
            <div>
              <p className="text-lg font-semibold">
                Have an event or moment to share?
              </p>

              <p className="mt-0.5 text-sm text-white/60">
                Send it our way and be part of the ALDC story.
              </p>
            </div>
          </div>
