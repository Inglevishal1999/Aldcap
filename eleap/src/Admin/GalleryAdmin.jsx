...
function resolveImage(path) {
  ...
  return `https://elaap-backend-live.onrender.com${path}`;
}
...
export default function GalleryAdmin() {
  ...
  async function handleDelete(id) {
    ...
    try {
      ...
      if (!json?.success) {
        throw new Error(
          json?.message || "Failed to delete item"
        );
      }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!json?.success) {
        throw new Error(
          json?.message || "Failed to save item"
        );
      }
          ...
          <div>
            ...
            {isEditing && (
              <p className="mt-1.5 text-xs text-slate-500">
                Leave as-is to keep current image, or
                choose new one.
              </p>
            )}
